    ...
    <style>
        * {
        ...
        .visual-card img {
            ...
            border-radius: 20px;
            filter:
                drop-shadow(0 12px 24px rgba(15, 23, 42, 0.35)) brightness(1.25) contrast(1.12);
        }
        ...
        .group select.auth-input~label {
            top: -12px;
            font-size: 11px;
            color: #f97316;
        }

        /* Fix for RT/RW small labels - add padding top to inputs */
        .group .auth-input,
        .group .auth-textarea,
        .group select.auth-input {
            padding-top: 8px;
        }
...
<body class="theme-dark">
            ...
            <div class="auth-panel">
                <div class="auth-panel-inner">
                    ...
                    <p class="auth-subtitle">
                        Isi data toko dan data diri Anda sebagai pemilik/penanggung jawab toko. Pengajuan akan diperiksa
                        oleh admin sebelum toko aktif.
                    </p>

                    @if(session('info'))
                        <div
                            style="background:rgba(59,130,246,0.15);border:1px solid rgba(96,165,250,0.5);border-radius:10px;padding:14px 18px;margin-bottom:18px;">
                            <div style="display:flex;align-items:start;gap:10px;">
                                <i class="uil uil-info-circle" style="font-size:22px;color:#60a5fa;margin-top:2px;"></i>
                                <div style="flex:1;">
                                    <div style="font-weight:700;color:#93c5fd;margin-bottom:4px;font-size:14px;">Informasi
                                    </div>
                    ...
                    <form method="POST" action="{{ route('seller.register.store') }}" enctype="multipart/form-data"
                        ...
                        <div class="step step-1 step--active">
                            ...
                            <div class="field-group group">
                                <input id="nama_toko" type="text" name="nama_toko" class="auth-input"
                                    value="{{ old('nama_toko') }}" required placeholder=" ">
                            ...
                            {{-- Deskripsi singkat --}}
                            <div class="field-group group">
                                <textarea id="deskripsi_singkat" name="deskripsi_singkat" class="auth-textarea" required
                                    placeholder=" ">{{ old('deskripsi_singkat') }}</textarea>
                                <span class="highlight"></span>
                                <span class="bar"></span>
                            ...
                            <div class="field-row">
                                <div class="field-group group">
                                    <input id="nama_pic" type="text" name="nama_pic" class="auth-input"
                                        value="{{ old('nama_pic', Auth::user()->name) }}" required placeholder=" ">
                                    <span class="highlight"></span>
                                    <span class="bar"></span>
                                ...
                                <div class="field-group group">
                                    <input id="no_hp_pic" type="text" name="no_hp_pic" class="auth-input"
                                        value="{{ old('no_hp_pic') }}" required placeholder=" ">
                                    <span class="highlight"></span>
                                    <span class="bar"></span>
                            ...
                            {{-- No KTP (wajib) --}}
                            <div class="field-group group">
                                <input id="no_ktp_pic" type="text" name="no_ktp_pic" class="auth-input"
                                    value="{{ old('no_ktp_pic') }}" required placeholder=" ">
                                <span class="highlight"></span>
                                <span class="bar"></span>
                            ...
                            {{-- Email --}}
                            <div class="field-group group">
                                <input id="email_pic" type="email" name="email_pic" class="auth-input"
                                    value="{{ old('email_pic', Auth::user()->email) }}" required placeholder=" ">
                                <span class="highlight"></span>
                                <span class="bar"></span>
                            ...
                            {{-- Alamat Lengkap --}}
                            <div class="field-group group">
                                <textarea id="alamat_pic" name="alamat_pic" class="auth-textarea" required
                                    placeholder=" ">{{ old('alamat_pic') }}</textarea>
                                <span class="highlight"></span>
                                <span class="bar"></span>
                            ...
                            <div class="field-row">
                                <div class="field-group group">
                                    <input id="rt" type="text" name="rt" class="auth-input" value="{{ old('rt') }}"
                                ...
                                <div class="field-group group">
                                    <input id="rw" type="text" name="rw" class="auth-input" value="{{ old('rw') }}"
                                        required placeholder=" ">
                                    <span class="highlight"></span>
                                    <span class="bar"></span>
                            ...
                            <div class="field-row">
                                <div class="field-group group">
                                    <select id="provinsi" name="provinsi" class="auth-input" required>
                                ...
                                <div class="field-group group">
                                    <select id="kota" name="kota" class="auth-input" required>
                            ...
                            {{-- Kelurahan --}}
                            <div class="field-group group">
                                <select id="kelurahan" name="kelurahan" class="auth-input" required>
                        ...
                        <div class="step step-2">
                            ...
                            <div class="file-group">
                                ...
                                <input type="file" name="foto_pic" class="file-input" accept=".jpg,.jpeg,.png" required>
                            ...
                            <div class="file-group">
                                ...
                                <input type="file" name="file_ktp_pic" class="file-input" accept=".pdf,.jpg,.jpeg,.png"
                                    required>
    ...
    <script>
        ...
        const dataCache = { provinces: null, regencies: {}, villages: {} };
        ...
        function setLoading(el, loading) {
            el.disabled = loading;
            if (loading) el.innerHTML = '<option value="">Loading...</option>';
        }

        async function loadProvinsi() {
            if (dataCache.provinces) return populateProvinsi(dataCache.provinces);
            const provinces = await fetchData(`${API_BASE}/provinces.json`);
            dataCache.provinces = provinces;
            populateProvinsi(provinces);
        }
        ...
        provinsiSelect.addEventListener('change', async function () {
            ...
            if (!id) return;
            if (dataCache.regencies[id]) return populateKota(dataCache.regencies[id]);
            setLoading(kotaSelect, true);
            const regencies = await fetchData(`${API_BASE}/regencies/${id}.json`);
            dataCache.regencies[id] = regencies;
        ...
        kotaSelect.addEventListener('change', async function () {
            ...
            kelurahanSelect.innerHTML = '<option value="">Pilih Kelurahan/Desa</option>';
            if (!id) return;
            if (dataCache.villages[id]) return populateKelurahan(dataCache.villages[id]);
            setLoading(kelurahanSelect, true);
            // Fetch all kecamatan first, then all kelurahan
            const districts = await fetchData(`${API_BASE}/districts/${id}.json`);
            let allVillages = [];
            for (const d of districts) {
                const villages = await fetchData(`${API_BASE}/villages/${d.id}.json`);
                allVillages = allVillages.concat(villages);
            }
            allVillages.sort((a, b) => a.name.localeCompare(b.name));
            dataCache.villages[id] = allVillages;
            populateKelurahan(allVillages);
